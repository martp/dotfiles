...
# ── 7. SSH Keys ───────────────────────────────────────────────
step "SSH Keys"
info "You'll need to set up SSH keys manually:"
echo ""
echo "  Option A — Generate fresh keys (recommended for new Mac):"
echo "    ssh-keygen -t ed25519 -C \"mart@blewpri.co.uk\" -f ~/.ssh/id_ed25519"
echo "    ssh-keygen -t ed25519 -C \"mart@blewpri.co.uk\" -f ~/.ssh/id_ed25519_gitlab"
echo "    Then add the public keys to GitHub and GitLab."
echo ""
echo "  Option B — Copy keys from old Mac:"
echo "    scp oldmac:~/.ssh/id_ed25519* ~/.ssh/"
echo "    chmod 600 ~/.ssh/id_ed25519 ~/.ssh/id_ed25519_gitlab"
echo ""
echo "  After keys are in place, run:"
echo "    ssh-add --apple-use-keychain ~/.ssh/id_ed25519"
echo "    ssh-add --apple-use-keychain ~/.ssh/id_ed25519_gitlab"
echo "    ssh -T git@github.com   # test"
echo "    ssh -T git@gitlab.com   # test"
